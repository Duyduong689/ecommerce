...
const HotelPhotoGallery: FC<{ photos: ImageType[] }> = ({ photos }) => {
    ...
    const handleNext = () => {
        setCurrentPhotoIndex(nextIndex => nextIndex === photos.length - 1 ? 0 : nextIndex + 1)
    }

    const maximumVisiblePhotos = 3;
    const totalPhotos = photos.length;

    const displayPhotos = [...photos].slice(1, maximumVisiblePhotos == photos.length ? maximumVisiblePhotos : maximumVisiblePhotos - 1)
            ...
            <div className=" grid md:grid-cols-2 relative gap-5 px-3">
                <div className=" h-[540px] relative rounded-2xl overflow-hidden">
                    <div className=" hidden md:flex justify-center items-center w-full h-full">
                        <Image src={photos[0].url} alt={`Room Photo ${currentPhotoIndex + 1} `}
                            className=" img"
                            width={150}
                            height={150}
                ...
                <div className=" hidden md:grid grid-cols-2 h-full gap-5">
                    {displayPhotos.map((photo, index) =>
                        <div key={index} className=" cursor-pointer h-64 overflow-hidden rounded-2xl">
                            <Image src={photo.url} alt={`Room Photo ${index + 2}`} height={150} width={150} className=" img scale-animation" />
                        </div>)}
                    {remainingPhotosCount > 0 && <div className=" cursor-pointer relative h-64 rounded-2xl overflow-hidden" onClick={openModal.bind(this, maximumVisiblePhotos)}>
                        <Image src={photos[maximumVisiblePhotos - 1].url} alt={`Room Photo ${maximumVisiblePhotos}`} width={150} height={150} className="img" />
                        <div className=" absolute cursor-pointer text-white inset-0 flex items-center justify-center bg-[rgba(0,0,0,0.5)]">+ {remainingPhotosCount}</div>
                    </div>}
                </div>
                ...
                {showModal && <div className=" fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-90 z-[55]">
                    <div className=" h-[75vh] w-[320px] md:w-[700px] relative">
                        ...
                        <button className=" absolute top-2 right-2 text-white text-lg">
                            <MdCancel className=" font-medium text-2xl text-tertiary-dark" onClick={closeModal} />
                        </button>
                    </div>
                </div>
                }
